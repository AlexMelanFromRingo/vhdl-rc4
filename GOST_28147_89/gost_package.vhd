...
package gost_package is
    ...
    -- Таблиця замін зберігається ПЛОСКО: sbox(16*i + n) - це S_i[n].
    -- Вкладений масив (array of array) змушує синтезатор трактувати
    -- звертання як частковий доступ до пам'яті; плоский масив дає
    -- звичайне ПЗП і синтезується без проблем.
    type sbox_t is array (0 to 127) of nibble_t;
    ...
    -- Допоміжні типи для запису таблиць у звичному вигляді 8 x 16
    type int_row_t is array (0 to 15) of integer;
    type int_box_t is array (0 to 7)  of int_row_t;
    function make_sbox(v : int_box_t) return sbox_t;
...
package body gost_package is
    ...
    function make_sbox(v : int_box_t) return sbox_t is
        variable r : sbox_t;
    begin
        for i in 0 to 7 loop
            for n in 0 to 15 loop
                r(16*i + n) := to_unsigned(v(i)(n), 4);
            end loop;
        end loop;
        return r;
    end function make_sbox;

    constant S_TEST : sbox_t := make_sbox((
    ...
    constant S_CRYPTOPRO_A : sbox_t := make_sbox((
    ...
    constant S_TC26_Z : sbox_t := make_sbox((
    ...
    begin
        ...
        for i in 0 to 7 loop                          -- підстановка по півбайтах
            v(4*i+3 downto 4*i) := sbox(16*i + to_integer(t(4*i+3 downto 4*i)));
        end loop;
